...
function isNavigationBranchItem(item: NavigationMenuItem): item is Exclude<NavigationMenuItem, string> & { items: NavigationMenuItem[] } {
...
function navigationItemsContainLeaf(items: NavigationMenuItem[], leafLabel: string): boolean {
  return items.some((item) => {
    if (isNavigationBranchItem(item)) {
      return navigationItemsContainLeaf(item.items, leafLabel);
    }
...
function ensureGeneralLedgerPinnedLeafs(groups: NavigationGroup[]): NavigationGroup[] {
  return groups.map((group) => {
    if (group.label !== "General Ledger") {
      return group;
    }

    const pinnedLeafLabels = new Set(generalLedgerPinnedLeafs.map(getNavigationItemLabel));
    const missingLeafs = generalLedgerPinnedLeafs.filter((leaf) => !navigationItemsContainLeaf(group.items, getNavigationItemLabel(leaf)));
...
export function DashboardPage({ session, activeStoreId, workspaceId, onSelectStore, onSignOut }: DashboardPageProps) {
  ...
  const [workspaceSnapshots, setWorkspaceSnapshots] = useState<{
    ...
    serviceRows: ServiceWorkspaceRow[];
  }>({ partsRows: [], salesRows: [], serviceRows: [] });
    ...
    getWorkspace(activeStore.id, workspaceId)
      .then((payload) => {
        if (!ignore) {
          ...
          if (payload.workspaceId === "service") {
            setWorkspaceSnapshots((current) => ({ ...current, serviceRows: payload.rows }));
          } else if (payload.workspaceId === "parts") {
            setWorkspaceSnapshots((current) => ({ ...current, partsRows: payload.rows }));
          } else if (payload.workspaceId === "sales") {
            setWorkspaceSnapshots((current) => ({ ...current, salesRows: payload.rows }));
          }
  ...
  function isMenuItemVisible(groupLabel: string, item: string) {
    ...
    return hasExplicitWorkspaceAssignment(groupLabel, item);
  }
...
function renderWorkspace(
  ...
  switch (activeWorkspaceId) {
    ...
    case "boatInventory": {
      return <BoatInventoryWorkspace storeId={auditControls.activeStoreId} />;
    }
    case "analytics": {
      ...
      if (activeGeneralLedgerPage === "profitLoss") {
        return <ProfitLossWorkspace />;
      }
    ...
    case "reports": {
      return (
        <ReportCenterWorkspace
          auditLog={taskControls.activityEntries}
          partsRows={reportContext.snapshots.partsRows}
          salesRows={reportContext.snapshots.salesRows}
          serviceRows={reportContext.snapshots.serviceRows}
...
function ServiceRepairWorkbench(props: ServiceRepairWorkbenchProps) {
  ...
  const [isPartsPoOpen, setIsPartsPoOpen] = useState(false);
  const [partsPoVendor, setPartsPoVendor] = useState("Mercury Marine Parts");
  ...
  useEffect(() => {
    if (!selectedServiceRow) {
      setSchemaHealth("Snapshot-backed");
      return;
    }

    let ignore = false;
    void getNormalizedServiceOrder(props.storeId, selectedServiceRow.roNumber)
      .then((payload) => {
        if (!ignore) {
          setSchemaHealth(payload.source === "normalized" ? "Normalized-ready" : "Snapshot-backed");
        }
      })
      .catch(() => {
        if (!ignore) {
  ...
  }, [props.storeId, selectedServiceRow]);

  async function handleNormalizeServiceSchema() {
    if (!selectedServiceRow || isNormalizingSchema) {
      return;
    }

    setIsNormalizingSchema(true);
    try {
      const result = await normalizeServiceOrder(props.storeId, selectedServiceRow.roNumber);
      setSchemaHealth("Normalized-ready");
      setWorkbenchNotice(
        result.createdJobs > 0
          ? `Normalized ${result.createdJobs} service job${result.createdJobs === 1 ? "" : "s"} while preserving detailSnapshot.`
          : "RO data is already normalized-ready. detailSnapshot preserved."
      );
    } catch {
      setWorkbenchNotice("Normalize scaffold is ready; API normalization is unavailable for this local row.");
    } finally {
      setIsNormalizingSchema(false);
    }
  }
        ...
        <>
          ...
          <section className="legacy-service-pane-section">
            ...
            <div style={{ marginTop: 8 }}>
              <button className="legacy-task-status-button" onClick={() => setIsPartsPoOpen((o) => !o)} type="button">📦 Create PO for Backordered</button>
            </div>
            {isPartsPoOpen && (() => {
              const backorderedParts = selectedJob.parts.filter((p) => (p.available ?? 1) === 0);
              return (
                <div className="legacy-parts-po-panel">
                  <div className="legacy-parts-po-header">
                    <span>Backordered Parts PO</span>
                    <button className="legacy-task-status-button" onClick={() => setIsPartsPoOpen(false)} type="button">✕</button>
                  </div>
                  {backorderedParts.length === 0 ? (
                    <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.82rem" }}>No backordered parts found.</div>
                  ) : (
                    <>
                      <table className="legacy-parts-po-table">
                        <thead><tr><th>Part #</th><th>Description</th><th>Qty</th></tr></thead>
                        <tbody>
                          {backorderedParts.map((p) => (
                            <tr key={p.partNumber}>
                              <td>{p.partNumber}</td>
                              <td>{p.description}</td>
                              <td>{p.quantity}</td>
                      ...
                      <label className="workflow-field" style={{ marginTop: 8 }}>
                        <span>Vendor</span>
                        <select onChange={(e) => setPartsPoVendor(e.target.value)} value={partsPoVendor}>
                          {["Mercury Marine Parts", "Sea Ray Distribution", "West Marine Wholesale", "BRP Marine Parts"].map((v) => <option key={v}>{v}</option>)}
                        </select>
                      </label>
                      <button className="legacy-task-status-button" onClick={() => { setIsPartsPoOpen(false); }} style={{ marginTop: 8 }} type="button">
                        Create PO for {backorderedParts.length} backordered part{backorderedParts.length === 1 ? "" : "s"}
                      </button>
                    </>
                  )}
                </div>
...
<body>
  ...
  <div class="footer">Premier Marine — Thank you for your business!</div>
      ...
      function saveAsPdf() {
        ...
        document.title = `Invoice-RO${model?.roNumber ?? ""}`;
        window.print();
      ...
      paneContent = (
        <div className="legacy-service-tab-screen">
          <div className="legacy-invoice-print-area" id="invoice-print-area">
          <div className="legacy-invoice-layout" style={{ padding: "16px", overflowY: "auto" }}>
            ...
            <section className="legacy-info-card legacy-invoice-header">
              ...
              <div className="legacy-invoice-actions">
                ...
                <button className="legacy-task-status-button" onClick={saveAsPdf} type="button">💾 Save as PDF</button>
  ...
  return (
    <section className="legacy-info-card legacy-service-workbench is-lightspeed-layout">
      ...
      <div className="legacy-schema-health-panel">
        <div>
          <span>Schema Health</span>
          <strong>{schemaHealth}</strong>
          <small>Relational job/parts/labor/sublet scaffold preserves the existing detailSnapshot.</small>
        </div>
        <button
          className="legacy-task-status-button"
          disabled={isNormalizingSchema || !selectedServiceRow}
          onClick={() => void handleNormalizeServiceSchema()}
          type="button"
        >
          {isNormalizingSchema ? "Normalizing..." : "Normalize RO Data"}
        </button>
      </div>
...
function SalesDealWorkbench({
  ...
  const [selectedFiProductIds, setSelectedFiProductIds] = useState<string[]>(["warranty"]);
  ...
  const selectedFiProducts = fiProductLibrary.filter((product) => selectedFiProductIds.includes(product.id));
  const fiProductRetailTotal = selectedFiProducts.reduce((total, product) => total + product.retail, 0);
  const fiProductMarginTotal = selectedFiProducts.reduce((total, product) => total + product.retail - product.cost, 0);
        ...
        return (
          <div className="legacy-fi-panel">
            ...
            <div className="legacy-fi-results">
              <div className="legacy-fi-products-deal">
                <h4 className="legacy-fi-section-title">F&amp;I Products</h4>
                <div className="legacy-fi-products-list">
                  {fiProductLibrary.map((product) => {
                    const isSelected = selectedFiProductIds.includes(product.id);
                    return (
                      <label className={`legacy-fi-products-option${isSelected ? " is-selected" : ""}`} key={product.id}>
                        <input
                          checked={isSelected}
                          onChange={() =>
                            setSelectedFiProductIds((current) =>
                        ...
                        <span>
                          ...
                          <small>{product.provider} · {product.term} · {formatCurrencyCompact(product.retail)}</small>
                        </span>
                      </label>
                    );
                  })}
                </div>
                <div className="legacy-fi-products-totals">
                  <span>Product Retail <strong>{formatCurrencyCompact(fiProductRetailTotal)}</strong></span>
                  <span>Margin Contribution <strong>{formatCurrencyCompact(fiProductMarginTotal)}</strong></span>
                </div>
              </div>
